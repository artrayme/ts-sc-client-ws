...
import {ScEvent, ScEventParams, ScEventType, ScTemplate, ScTemplateResult} from "../src";
...
describe("Sc-Client with real sc-machine server", () => {
    ...
    test("templateSearchBySCs", async () => {
        ...
        const nodeAddrs = await client.createElements(prepareConstruction);
        const nodeAddr1 = nodeAddrs[0];
        const nodeAddr2 = (await client.resolveKeynodes([{id: "node_for_client_testing", type: ScType.NodeConst}])).node_for_client_testing
        const construction = new ScConstruction();
        construction.createEdge(ScType.EdgeAccessConstPosPerm, nodeAddr2, nodeAddr1);
        const edgeAddrs = await client.createElements(construction);
